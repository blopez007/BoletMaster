<template>
  <v-container class="white">
          ...
          <v-container class="pa-0">
            ...
            <v-row>
              <v-col cols="12" md="6">
                ...
                <v-file-input v-model="archivo" :rules="rules" accept=".png, .jpg" placeholder="Seleccione Banner"
                  prepend-icon="mdi-camera" label="Banner del Evento" style="width: 100%" outlined></v-file-input>
                <v-img :src="previaImagen" style="" min-height="10vh" min-width="100%">
                  <p v-if="!archivo" class="ma-5"></p>
                </v-img>
              </v-col>

              <v-col cols="12" md="6">
                <v-combobox multiple :item-value="(categoria) => categoria.id"
                  :item-text="(categoria) => categoria.name" v-model="categoriasSeleccionadas" :items="category"
                  label="Categoria" prepend-icon="mdi-playlist-minus" outlined></v-combobox>
    ...
    <div class="text-center">
      <v-btn :disabled="!valid" color="#1A374D" class="mr-4" @click="guardarevento()">
...
export default {
  ...
  methods: {
    ...
    cancelar() {
      this.$confirm(
        "Estas seguro de querer cancelar el proceso?.",
        "Question",
        "question"
      ).then((resultado) => {
        if (resultado === true) {
    ...
    async guardarevento() {
      ...
      var response = await HTTP.post("/events/create", data, {
        headers: {
      ...
      this.$alert("Evento Añadido", "Aviso", "Succes", {
        confirmButtonText: "Cerrar",
      });
      this.$router.push("/administrador")
    },
  ...
  computed: {
    previaImagen() {
      if (this.archivo) return URL.createObjectURL(this.archivo);
    },
  },
};
</script>
